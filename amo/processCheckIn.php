...
foreach ($leadsArray as $lead) {
    ...
    try {
        ...
        // Добавьте до проверки условия
        file_put_contents($logFile, "[" . date('Y-m-d H:i:s') . "] DEBUG: Timestamp: $checkInDateValues[0], Date в UTC: " . date('Y-m-d', $checkInDateValues[0]) . ", Date в Europe/Moscow: " . date('Y-m-d', $checkInDateValues[0] + (3*3600)) . "\n", FILE_APPEND);
        ...
        if (is_numeric($checkInDate)) {
            // Устанавливаем временную зону UTC для корректной конвертации
            $originalTimeZone = date_default_timezone_get();
            date_default_timezone_set('UTC');
            // Преобразуем timestamp в формат YYYY-MM-DD
            $checkInDate = date('Y-m-d', $checkInDate);
            // Возвращаем исходную временную зону
